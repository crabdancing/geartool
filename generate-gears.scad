...
if (gear_type == "spur") {
	...
	planetary_gear(modul = mod, sun_teeth = num_sun_tooth_P, planet_teeth = num_planet_tooth_P, number_planets = num_planets_P, width = gear_width_P, rim_width = rim_width_P, planet_bore = planet_bore_P, sun_bore = sun_bore_P, pressure_angle = pressure_angle, helix_angle = helix_angle, together_built = together_built, optimized=true, $fn = $fn);
} else {
	assert(false);
}